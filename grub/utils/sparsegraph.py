...
from pgnn.configuration.experiment_configuration import ExperimentConfiguration, AttributeNormalization
...
class SparseGraph:
    ...
    def normalize_features(self, experiment_configuration: ExperimentConfiguration):
        if experiment_configuration.binary_attributes:
            self.attr_matrix[self.attr_matrix > 0] = 1

        if experiment_configuration.normalize_attributes == AttributeNormalization.DEFAULT:
            self.attr_matrix = normalize_attributes(self.attr_matrix)
            self.attr_matrix = matrix_to_torch(self.attr_matrix).to(get_device())
        elif experiment_configuration.normalize_attributes == AttributeNormalization.DIV_BY_SUM:
            self.attr_matrix = matrix_to_torch(self.attr_matrix).to(get_device())
            self.attr_matrix = self.attr_matrix / (self.attr_matrix.sum(dim=-1).unsqueeze(-1) + 1e-10)
        elif experiment_configuration.normalize_attributes == AttributeNormalization.NONE:
            self.attr_matrix = matrix_to_torch(self.attr_matrix).to(get_device())
        else:
